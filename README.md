...
- **本の登録**: タイトル・著者名を含め、すべての項目を未入力のまま登録できます(2026-08-07変更)。未入力の場合、タイトルは「タイトル未設定」、著者名は「著者不明」としてデータベースに保存されます。ただし書籍情報(タイトル・著者名など)は登録後の編集画面からは変更できない仕様のままなので、あとから正式なタイトルを入れたい場合は再登録が必要です。
- **友達申請ボタンのローディング表示バグ修正(2026-08-07)**: 「メンバー一覧」「ユーザー検索」タブの「申請」ボタンに、送信中のローディング表示・二重送信防止のdisabled制御が入っておらず、通信に時間がかかると「押しても反応がない」ように見える不具合がありました。実際には申請自体は成功していましたが、UI上のフィードバックが不足していたため修正しました。
- **通知が作成されない不具合修正(2026-08-10)**: `notifications`テーブルにINSERT(作成)を許可するRLSポリシーが1つもなく、友達申請・承認などの通知作成処理が常に失敗していました(`supabase/migrations/0004_notifications_insert_policy.sql`で修正)。既存のSupabaseプロジェクトをお使いの場合は、このマイグレーションを追加で実行してください。
